...
class DeepQNetwork:
    ...
    def store_transition(self, s, a, r, s_):
        ...
    def choose_action(self, observation):
        # 统一 observation 的 shape (1, size_of_transition)
...
class DeepQNetwork:
    ...
    def learn(self):
        ...
        # 下面是将 q_eval 全部赋值给 q_target, 这时 q_target-q_eval = 0,
        ...
        """
        加入在这个 batch 中, 我们有2个提取的记忆, 根据每个记忆可以生产3个 action 的值:
        q_eval =
        [[1, 2, 3],
         [4, 5, 6]]
        ...
        比如在:
            记忆 0, 我是用了 action 0, 这个 action 0 的 q_target 计算值是 -1;
            记忆 1, 我是用了 action 2, 这个 action 2 的 q_target 计算值是 -2:
        q_target =
        [[-1, 2, 3],
         [4, 5, -2]]
        ...
        最后我们在将这个 (q_target - q_eval) 当成误差, 反向传递会神经网络.
        为 0 的所有对应的 action 值意思就是, 我们只反向传递之前选择的 action 的值,
        没有选择的不传递.
        """
