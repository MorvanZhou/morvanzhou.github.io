...
这次我们使用一个连续动作的环境举例, 直接看所有代码, [请看我的 Github](https://github.com/MorvanZhou/tutorials/blob/master/Reinforcement_learning_TUT/10_A3C/A3C_continuous_action.py),
...
# 也能被调用生成一个 worker 的 net, 因为他们的结构又是一样的,
# 所以这个 class 可以被重复利用.
class ACNet(object):
        ...
        if 这是 global:   # 判断当下建立的网络是 local 还是 global
            ...
            with tf.name_scope('sync'):  # 同步
                with tf.name_scope('pull'): # 更新去 global
                    # 调用 globalAC 中的所有参数,
                    # 并将刚刚计算的 gradients 施加到 global net 去
                with tf.name_scope('push'): # 获取 global 参数
                    # 调用 globalAC, 并将自己的 local 参数全部同步成 globalAC 的参数

    def _build_net(self):
        # 在这里搭建 Actor 和 Critic 的网络
        return 均值, 方程, state_value
